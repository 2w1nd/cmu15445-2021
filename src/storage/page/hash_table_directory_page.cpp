...
/*
 * bucket_idx = 3(011), local_depth = 2
 * LHB = 100
 * 100 | (11 & 01) = 101
 * 101 & 011 = 001
 * */
...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {
  ...
  uint32_t split_image_idx = high_bits | (bucket_idx & (Pow(2, GetLocalDepth(bucket_idx) - 1) - 1));
  return split_image_idx & GetLocalDepthMask(bucket_idx);
}
...
uint32_t HashTableDirectoryPage::Pow(uint32_t base, uint32_t power) {
